...
Astar1A007 = Astar / A007;

Astar1A007fsolve = @(M) ( (((gamma+1)/2)/( 1+ (gamma-1)/2 * M*M ))^((gamma+1)/(2*(gamma-1)))*M) - Astar1A007;
M_sh1 = fsolve(Astar1A007fsolve,1.2,options);

M_sh2 = (1 + (gamma - 1)/2*M_sh1^2 ) / (gamma*M_sh1^2 - (gamma - 1)/2 );
Astar2 = A007 * ( ( ((gamma+1)/2) / (1+(gamma-1)/2 * M_sh2*M_sh2) )^((gamma+1)/(2*(gamma-1))) * M_sh2 );
Astar2Aduct = Astar2 / Aduct;

Astar2Aductfsolve = @(M) ( (((gamma+1)/2)/( 1+ (gamma-1)/2 * M*M ))^((gamma+1)/(2*(gamma-1)))*M) - Astar2Aduct;
M_ind2 = fsolve(Astar2Aductfsolve,0.5,options);
